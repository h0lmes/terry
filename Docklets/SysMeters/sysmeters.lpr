...
//------------------------------------------------------------------------------
procedure getDrive(letter: char; var freeMB, freePercent: integer);
var
  free, total: Int64;
  totalMB: integer;
...
begin
  ...
  freePercent := round((totalMB - freeMB) / totalMB * 100);
end;
//------------------------------------------------------------------------------
...
begin
  ...
  if data.mode = mmDrive then
  begin
    getDrive(data.drive, value, value);
    if value > 100 then value := 100;
    if value >= 0 then
    begin
...
begin
  ...
  begin
    GetPrivateProfileString(szIniGroup, 'mode', inttostr(integer(mmCPU)), @szRet, MAX_PATH, szIni);
    try Instance.mode := TMeterMode(inttostr(strpas(@szRet)));
    except end;
  end;
