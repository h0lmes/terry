...
uses Windows, Classes, SysUtils, Graphics, ActiveX, GDIPAPI, toolu;
...
begin
  ...
  try
    ...
    if not isFullyQalified or not fileexists(imagefile) then
    begin
      if default then LoadGdipImageFromResource('DEFAULT_ICON', image);
    end else begin
      ext:= AnsiLowerCase(ExtractFileExt(imagefile));
      try
        if (ext = '.png') or (ext = '.gif') then
        begin
          {fstream:= TFileStream.Create(cut(imagefile, ','), fmOpenRead);
          stream:= TStreamAdapter.Create(fstream, soReference);
...
begin
  ...
  GdipLoadImageFromStream(s, image);
  rs.free;
end;
//------------------------------------------------------------------------------
function SwapColor(color: uint): uint;
